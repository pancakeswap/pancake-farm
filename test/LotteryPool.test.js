...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.chef = await MasterChef.new(this.cake.address, this.syrup.address, dev, '10', '10', { from: minter });
    ...
    it('real case', async () => {
        this.lottery = await LotteryRewardPool.new(this.chef.address, this.cake.address, dev, carol, { from: minter });
        ...
        assert.equal((await this.cake.balanceOf(this.lottery.address)).toString(), '0');

        await this.lottery.harvest(4, { from: dev })
        // console.log(await this.lottery.pendingReward(4).toString())

        assert.equal((await this.cake.balanceOf(this.lottery.address)).toString(), '0');
        assert.equal((await this.cake.balanceOf(carol)).toString(), '22');
