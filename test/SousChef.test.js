...
contract('SousChef', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.syrup = await MockBEP20.new('LPToken', 'LP1', '1000000', { from: minter });
    this.chef = await SousChef.new(this.syrup.address, '40', '300', '400', { from: minter });
  });
  ...
  it('sous chef now', async () => {
    ...
    assert.equal((await this.syrup.balanceOf(this.chef.address)).toString(), '10');

    await time.advanceBlockTo('300');

    await this.chef.deposit('30', { from: alice });
    assert.equal((await this.syrup.balanceOf(this.chef.address)).toString(), '40');
    ...
    assert.equal((await this.syrup.balanceOf(this.chef.address)).toString(), '80');
  ...
  it('try syrup', async () => {
    ...
    this.chef = await MasterChef.new(this.cake.address, this.syrup.address, dev, '1000', '300', { from: minter });
    ...
    this.chef2 = await SousChef.new(this.syrup.address, '40', '600', '800', { from: minter });
    ...
    assert.equal((await this.syrup.balanceOf(this.chef2.address)).toString(), '10');
